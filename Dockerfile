...
RUN git clone --branch 1.33.0 https://github.com/input-output-hk/cardano-node.git && \
    cd cardano-node && \
...
COPY --from=builder /build/cardano-node/cardano-node/dist-newstyle/build/x86_64-linux/ghc-8.10.4/cardano-node-1.33.0/x/cardano-node/build/cardano-node/cardano-node /usr/local/bin/
COPY --from=builder /build/cardano-node/cardano-node/dist-newstyle/build/x86_64-linux/ghc-8.10.4/cardano-cli-1.33.0/x/cardano-cli/build/cardano-cli/cardano-cli /usr/local/bin/
